 ...
 * Get current version
 */
export async function getVersion(): Promise<{ version: string; image_version?: string }> {
  const { data } = await apiClient.get<{ version: string; image_version?: string }>('/admin/system/version')
  return data
}
