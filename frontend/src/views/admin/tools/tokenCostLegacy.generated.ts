...
function resolveTokenCostService(candidate: unknown) {
  ...
  if (
    ...
    throw new Error('Token 成本动态数据服务未注入')
  }
  return service
}
...
export function mountTokenCostLegacyTool(scope: LegacyToolScope): LegacyToolCleanup {
  ...
  const tokenCostService = resolveTokenCostService(scope.services?.tokenCost)
  const CSS = scope.CSS
  const console = scope.console

    const DEFAULT_PERSONAL_RECHARGE_R = 400;
    ...
    function normalizeState(source = {}) {
      ...
      merged.platforms = Array.isArray(sourceState.platforms) ? sourceState.platforms.map(normalizePlatform) : [];
      merged.history = Array.isArray(sourceState.history) ? sourceState.history : [];
      merged.events = Array.isArray(sourceState.events) ? sourceState.events : [];
      merged.personalRechargeR = toPositive(sourceState.personalRechargeR, DEFAULT_PERSONAL_RECHARGE_R);
      return merged;
    }
    ...
    async function checkServer() {
      try {
        await tokenCostService.health();
        serverAvailable = true;
        await loadStateFromServer(false);
      } catch (error) {
        serverAvailable = false;
        setStatus(`读取数据库失败：${error.message || error}`, "warn");
      }
    }

    async function loadStateFromServer(manual = false) {
      const latestState = await tokenCostService.loadState();
      state = normalizeState(latestState);
      serverAvailable = true;
      render();
      setStatus(manual ? "已从数据库重新载入最新数据。" : `已从数据库载入最新数据：${state.platforms.length} 个平台。`, "ok");
      pulseStats();
    }
    ...
    async function saveDataToServer(manual) {
      try {
        const savedState = await tokenCostService.saveState(normalizeState(state));
    ...
    async function resetToDefault() {
      if (!window.confirm("确认从数据库重新载入最新数据？当前页面未保存的动态改动会被覆盖。")) {
        return;
      }
    ...
    document.querySelector("#openReportBtn").addEventListener("click", () => loadStateFromServer(true));
