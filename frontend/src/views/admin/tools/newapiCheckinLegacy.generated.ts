    ...
    function renderConfig(config) {
      ...
      visibleSites.forEach((site) => {
        (site.accounts || []).forEach((account, index) => {
          ...
          if (readOnly) {
            apiKeyHtml = account.access_key_masked
              ? `<code title="sub2api 访问 Key">${escapeHtml(account.access_key_masked)}</code>`
