...
export const newapiCheckinReferencedAccountStyles = `
  .catalog-row.referenced-account td {
    background: color-mix(in oklab, var(--ok-soft) 42%, var(--surface));
  }

  .catalog-row.referenced-account td:first-child {
...
export function mountNewapiCheckinLegacyTool(scope: LegacyToolScope): LegacyToolCleanup {
  ...
  const console = scope.console
    ...
    const openCreateSiteBtn = document.getElementById("openCreateSiteBtn");
    const createSiteDialog = document.getElementById("createSiteDialog");
    const closeCreateSiteBtn = document.getElementById("closeCreateSiteBtn");
    const createSiteForm = document.getElementById("createSiteForm");
    const createSiteProviderInput = document.getElementById("createSiteProviderInput");
    const createSiteNameInput = document.getElementById("createSiteNameInput");
    const createSiteURLInput = document.getElementById("createSiteURLInput");
    const createSiteStatus = document.getElementById("createSiteStatus");
    const createSiteSubmitBtn = document.getElementById("createSiteSubmitBtn");
    ...
    const createAccountSiteInput = document.getElementById("createAccountSiteInput");
    const createNewAPIAccountFields = document.getElementById("createNewAPIAccountFields");
    const createSub2AccountFields = document.getElementById("createSub2AccountFields");
    const createAccountUserIDInput = document.getElementById("createAccountUserIDInput");
    const createAccountAccessKeyInput = document.getElementById("createAccountAccessKeyInput");
    const createAccountEmailInput = document.getElementById("createAccountEmailInput");
    const createAccountPasswordInput = document.getElementById("createAccountPasswordInput");
    ...
    function openCreateSite() {
      ...
      setInlineStatus(createSiteStatus, "平台创建后可继续添加账号。", "normal");
      if (typeof createSiteDialog.showModal === "function") {
        createSiteDialog.showModal();
      } else {
        createSiteDialog.setAttribute("open", "");
      }
      createSiteNameInput.focus();
    }

    function closeCreateSite() {
      if (typeof createSiteDialog.close === "function") {
        createSiteDialog.close();
      } else {
        createSiteDialog.removeAttribute("open");
      }
    }

    function updateCreateAccountFields() {
      const site = getConfigSite(createAccountSiteInput.value);
      const sub2 = siteProvider(site) === "sub2api";
      createNewAPIAccountFields.hidden = sub2;
      createSub2AccountFields.hidden = !sub2;
      createAccountUserIDInput.required = !sub2;
      createAccountAccessKeyInput.required = !sub2;
      createAccountEmailInput.required = sub2;
      createAccountPasswordInput.required = sub2;
      setInlineStatus(
        createAccountStatus,
        sub2 ? "Sub2API 会先验证登录，并自动读取完整 Key 与分组。" : "NewAPI 使用 User ID 和访问 Key。",
        "normal",
      );
    }
    ...
    function openCreateAccount() {
      ...
      if (!sites.length) {
        setAppStatus("请先新增平台，再添加账号。", "error");
        return;
      }
      createAccountForm.reset();
      createAccountSiteInput.innerHTML = sites.map((site) => `
        <option value="${escapeHtml(site.name)}">${escapeHtml(site.name)} · ${siteProvider(site)}</option>
      `).join("");
      if (state.activeSite !== "all" && sites.some((site) => site.name === state.activeSite)) {
        createAccountSiteInput.value = state.activeSite;
      }
      updateCreateAccountFields();
      if (typeof createAccountDialog.showModal === "function") {
        createAccountDialog.showModal();
      } else {
        createAccountDialog.setAttribute("open", "");
      }
      (createNewAPIAccountFields.hidden ? createAccountEmailInput : createAccountUserIDInput).focus();
    }

    function closeCreateAccount() {
      createAccountPasswordInput.value = "";
      if (typeof createAccountDialog.close === "function") {
    ...
    async function submitCreateSite() {
      const payload = {
        name: createSiteNameInput.value.trim(),
        provider: createSiteProviderInput.value,
        base_url: createSiteURLInput.value.trim(),
      };
      setInlineStatus(createSiteStatus, "正在创建平台…", "busy");
      setButtonBusy(createSiteSubmitBtn, true, "创建中");
      try {
        state.config = await fetchJson("/api/sites", {
          method: "POST",
        ...
        renderAllViews();
        closeCreateSite();
        setAppStatus(`平台 ${payload.name} 已创建`, "ok");
      } catch (error) {
        setInlineStatus(createSiteStatus, `创建失败: ${error.message}`, "error");
      } finally {
        setButtonBusy(createSiteSubmitBtn, false);
      }
    }
    ...
    async function submitCreateAccount() {
      const site = getConfigSite(createAccountSiteInput.value);
      if (!site) {
        setInlineStatus(createAccountStatus, "请选择有效平台。", "error");
        return;
      }
      const sub2 = siteProvider(site) === "sub2api";
      const payload = sub2
        ? {
            site: site.name,
            login_username: createAccountEmailInput.value.trim(),
            login_password: createAccountPasswordInput.value,
          }
        : {
            site: site.name,
            user_id: createAccountUserIDInput.value.trim(),
            access_key: createAccountAccessKeyInput.value.trim(),
          };
      setInlineStatus(createAccountStatus, sub2 ? "正在验证登录并读取 Key / 分组…" : "正在创建账号…", "busy");
      setButtonBusy(createAccountSubmitBtn, true, "创建中");
      try {
        state.config = await fetchJson("/api/accounts", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
        await Promise.all([loadAPIKeys(), loadBalances()]);
        renderConfigSelectors();
        renderAllViews();
        closeCreateAccount();
        setAppStatus(`${site.name} 的账号已创建`, "ok");
      } catch (error) {
        setInlineStatus(createAccountStatus, `创建失败: ${error.message}`, "error");
      } finally {
        setButtonBusy(createAccountSubmitBtn, false);
      }
    }
    ...
    // 统计账号内已被主平台数据库引用的 Key 数量，供账号级排序与标识复用。
    function referencedAPIKeyCount(apiKeySummary) {
      return (apiKeySummary?.api_keys || []).filter((item) => (
        (item.referenced_accounts || []).length > 0
        || (item.target_accounts || []).some((target) => target.referenced)
      )).length;
    }
    ...
    function renderConfig(config) {
      ...
      visibleSites.forEach((site) => {
        const sortedAccounts = (site.accounts || []).slice().sort((left, right) => {
          const leftSummary = apiKeyAccounts.get(`${site.name}\u0000${left.user_id}`);
          const rightSummary = apiKeyAccounts.get(`${site.name}\u0000${right.user_id}`);
          return Number(referencedAPIKeyCount(rightSummary) > 0) - Number(referencedAPIKeyCount(leftSummary) > 0);
        });
        sortedAccounts.forEach((account, index) => {
          const readOnly = isReadOnlySite(site);
          const apiKeySummary = apiKeyAccounts.get(`${site.name}\u0000${account.user_id}`);
          const referenceCount = referencedAPIKeyCount(apiKeySummary);
          ...
          rows.push({
            _rowClass: `catalog-row ${index === 0 ? "site-start" : ""} ${referenceCount > 0 ? "referenced-account" : ""}`,
            site: site.name,
            user_id: account.user_id,
            siteHtml,
            identity: account,
            referenceBadgeHtml: referenceCount > 0
              ? `<div class="account-reference-summary">${badge(`已引用 ${referenceCount} 个 Key`, "ok")}</div>`
              : "",
      ...
      renderTable(
        ...
        [
          ...
          { label: "名称 / 账号", value: (row) => `${identityHtml(row.identity)}${row.referenceBadgeHtml}` },
    ...
    async function refreshDirectoryKeys() {
      setButtonBusy(refreshDirectoryKeysBtn, true, "同步中");
      try {
        state.revealedGeneratedKeys.clear();
        state.apiKeys = await fetchJson("/api/api-keys/sync", { method: "POST" });
        renderConfig(state.config);
        setAppStatus(`API Key 与分组已全量同步，更新 ${state.apiKeys?.refreshed_count || 0} 个账号`, "ok");
      } catch (error) {
        setAppStatus(`同步 API Key 与分组失败: ${error.message}`, "error");
      } finally {
    ...
    openCreateSiteBtn.addEventListener("click", openCreateSite);
    closeCreateSiteBtn.addEventListener("click", closeCreateSite);
    createSiteForm.addEventListener("submit", (event) => {
    ...
    createAccountSiteInput.addEventListener("change", updateCreateAccountFields);
