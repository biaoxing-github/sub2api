...
const state = {
    ...
    function attachConfigActions() {
      Array.from(configTableWrap.querySelectorAll("[data-pick-site][data-pick-user]")).forEach((button) => {
        button.addEventListener("click", () => {
          if (!isSiteEnabled(button.dataset.pickSite)) {
            singleRunStatus.textContent = `${button.dataset.pickSite} 已停用签到，不能设为补签目标。`;
    ...
    function renderOverview() {
      ...
      if (!feedItems.length) {
        feedItems.push({
          ...
          body: "如果要补签个别账号，可以直接从平台目录里点“设为补签”。",
        });
      }
            ...
            action: `
              <div class="action-cluster">
                <button
                  ...
                  title="${site.enabled ? "设为单账号补签目标" : "站点已停用签到，不能补签"}"
                >
                  设为补签
                </button>
                <button
                ...
                ${index === 0 ? `
                  <button
                    ...
                    title="${site.enabled ? "禁用该站点后不会再执行签到" : "重新允许该站点参与签到"}"
                  >
                    ${site.enabled ? "禁用签到" : "启用签到"}
                  </button>
                  <button
                    class="quick-action danger"
                    type="button"
    ...
    function renderBalances(payload) {
      ...
      renderTable(
        ...
        [
          ...
          { label: "名称 / 账号", value: (row) => identityHtml(row) },
          { label: "IP 档位", value: (row) => `<span class="route-badge">${escapeHtml(row.ip_profile || "-")}</span>` },
          { label: "签到状态", value: (row) => badge(row.checkin_status || row.status, row.checkin_status_tone || (row.ok === false ? "danger" : "ok")) },
          { label: "余额($)", value: (row) => escapeHtml(formatDisplayValue(row.quota_display)) },
          { label: "已用($)", value: (row) => escapeHtml(formatDisplayValue(row.used_quota_display)) },
    ...
    async function loadLocalState() {
      ...
      try {
        ...
        await loadMonthly();
        renderAllViews();
        setAppStatus(`SQL 数据已读取 ${new Date().toLocaleTimeString("zh-CN", { hour12: false })}`, "ok");
      } catch (error) {
    ...
    async function refreshSiteBalance() {
      ...
      setAppStatus(`正在刷新 ${site} 的余额与签到状态…`, "busy");
      ...
      try {
        ...
        renderAllViews();
        setAppStatus(`${site} 已写入 SQL：签到成功 ${result.success_count}，今日已签 ${result.already_done_count}，异常 ${result.failed_count}`, result.failed_count > 0 ? "error" : "ok");
    ...
    async function refreshAccountBalance(site, userId, button) {
      setAppStatus(`正在刷新 ${site} / ${userId} 的余额与签到状态…`, "busy");
      ...
      try {
        ...
        state.monthly = result.monthly || state.monthly;
        renderAllViews();
        setAppStatus(`${site} / ${userId} 已写入 SQL：${result.checkin?.checkin_status || result.message || "状态已刷新"}`, result.ok ? "ok" : "error");
