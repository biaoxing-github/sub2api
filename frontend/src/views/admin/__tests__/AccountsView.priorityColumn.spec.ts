...
describe('admin AccountsView priority column preferences', () => {
  ...
  it('preserves an existing preference that explicitly hides priority', async () => {
    localStorage.setItem('account-hidden-columns-v2', JSON.stringify(['priority', 'today_stats']))

    const wrapper = mountView()
    await flushPromises()

    expect(wrapper.find('[data-column="priority"]').exists()).toBe(false)
    expect(JSON.parse(localStorage.getItem('account-hidden-columns-v2') || '[]')).toEqual([
      'priority',
      'today_stats'
    ])
  })

  it('keeps priority visible when legacy preferences are ignored by the v2 layout', async () => {
