      ...
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr
            v-for="(row, i) in rows"
            :key="i"
            class="cursor-pointer border-t border-gray-100 hover:bg-gray-50 dark:border-dark-700 dark:hover:bg-dark-800"
          ...
          >
            ...
            <td class="px-4 py-2">
              ...
              <span
                ...
                class="ml-1 inline-flex items-center rounded bg-gray-100 px-1 py-px text-[10px] font-medium leading-tight text-gray-500 dark:bg-dark-700 dark:text-gray-400"
            ...
            <td class="max-w-[280px] truncate px-4 py-2" :title="row.message">{{ row.message || '-' }}</td>
...
const modelOptions = computed(() => {
  ...
  const opts: { value: string; label: string }[] = [{ value: '', label: t('usage.errors.allModels') }]
