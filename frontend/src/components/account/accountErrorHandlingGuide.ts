import type {
  AccountErrorHandlingGuideContext,
  AccountErrorHandlingGuideField,
  AccountErrorHandlingGuideSource
} from './accountErrorHandlingTypes'

const emptyErrorPolicyRulesDescription = '未配置账号专属规则；需要特殊处理时再添加'

export function resolveAccountErrorHandlingContextGuide(_input: {
  accountType?: string
  baseUrl?: string
  providerCode?: string
}): AccountErrorHandlingGuideContext {
...
export const accountErrorHandlingGuideFields: AccountErrorHandlingGuideField[] = [
  {
    key: 'status_codes',
    field: '状态码',
    source: '上游 HTTP 状态码',
...
export const accountErrorHandlingGuideSources: AccountErrorHandlingGuideSource[] = [
  {
    key: 'audit',
    name: '原始审计日志',
