import {
  accountErrorHandlingActionValues,
  type AccountErrorHandlingAction,
  type AccountErrorHandlingRuleForm,
  type AccountErrorHandlingRulePayload,
  type AccountErrorHandlingResetStrategy
} from './accountErrorHandlingTypes'
import { makeAccountErrorHandlingRule } from './accountErrorHandlingRules'
...
const normalizeOptionalAction = (value: unknown): AccountErrorHandlingAction | null => {
  return accountErrorHandlingActionValues.includes(value as AccountErrorHandlingAction)
    ? value as AccountErrorHandlingAction
    : null
}

const normalizeOptionalRecoveryStrategy = (value: unknown): AccountErrorHandlingResetStrategy | null => {
...
function payloadAction(value: unknown): AccountErrorHandlingAction {
  if (!accountErrorHandlingActionValues.includes(value as AccountErrorHandlingAction)) throw new Error('错误处理动作无效')
  return value as AccountErrorHandlingAction
}

function payloadRecoveryStrategy(value: unknown): AccountErrorHandlingResetStrategy {
...
const buildRuleFromPayload = (value: unknown): AccountErrorHandlingRuleForm => {
  ...
  return makeAccountErrorHandlingRule({
...
export const loadAccountErrorHandlingRules = (credentials?: Record<string, unknown>): AccountErrorHandlingRuleForm[] => {
...
export const validateAccountErrorHandlingRules = (rules: AccountErrorHandlingRuleForm[]): { valid: boolean; message?: string; index?: number } => {
...
export const buildAccountErrorHandlingPayload = (rules: AccountErrorHandlingRuleForm[]): AccountErrorHandlingRulePayload[] => {
...
export const writeAccountErrorHandlingToCredentials = (credentials: Record<string, unknown>, rules: AccountErrorHandlingRuleForm[]): void => {
  const payload = buildAccountErrorHandlingPayload(rules)
  if (payload.length > 0) {
