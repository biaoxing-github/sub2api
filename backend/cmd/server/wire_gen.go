...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	userPlatformQuotaServiceAdapter := repository.NewUserPlatformQuotaServiceAdapter(userPlatformQuotaRepository)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, userPlatformQuotaServiceAdapter)
	...
	apiKeyProbeRepository := repository.NewAPIKeyProbeRepository(db)
	apiKeyProbeService := service.ProvideAPIKeyProbeService(apiKeyRepository, apiKeyProbeRepository)
	apiKeyProbeHandler := handler.NewAPIKeyProbeHandler(apiKeyProbeService)
	accountProbeRepository := repository.NewAccountProbeRepository(db)
	accountBatchTestRepository := repository.NewAccountBatchTestRepository(db)
	...
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, userPlatformQuotaServiceAdapter, timingWheelService)
	...
	contextJournal := service.ProvideContextJournal(redisClient, configConfig)
	upstreamBalanceService := service.ProvideUpstreamBalanceService(accountRepository, httpUpstream, configConfig)
	realtimeBalanceChecker := service.ProvideRealtimeBalanceChecker(upstreamBalanceService, configConfig)
	openAIRequestSnapshotRepository := repository.NewOpenAIRequestSnapshotRepository(db)
	openAIRequestSnapshotService := service.NewOpenAIRequestSnapshotService(openAIRequestSnapshotRepository, settingService)
	tlsFingerprintProfileRepository := repository.NewTLSFingerprintProfileRepository(client)
	tlsFingerprintProfileCache := repository.NewTLSFingerprintProfileCache(redisClient)
	tlsFingerprintProfileService := service.NewTLSFingerprintProfileService(tlsFingerprintProfileRepository, tlsFingerprintProfileCache)
	openAIGatewayService := service.NewOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, contextJournal, realtimeBalanceChecker, openAIRequestSnapshotService, tlsFingerprintProfileService, userPlatformQuotaServiceAdapter)
	...
	adminService.SetCompositeRouteDependencies(compositeModelRouteRepository, compositeRouteResolver)
	...
	accountTestService.SetSettingService(settingService)
	accountProbeService := service.NewAccountProbeService(accountRepository, accountProbeRepository, nil, accountTestService)
	accountProbeService.SetSettingService(settingService)
	...
	accountHandler.SetUsageService(usageService)
	...
	grokOAuthHandler := admin.NewGrokOAuthHandler(grokOAuthService)
	...
	gatewayService := service.NewGatewayService(accountRepository, groupRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, identityService, httpUpstream, deferredService, claudeTokenProvider, sessionLimitCache, rpmCache, digestSessionStore, settingService, tlsFingerprintProfileService, channelService, modelPricingResolver, balanceNotifyService, userPlatformQuotaServiceAdapter)
	adminService.SetGatewayService(gatewayService)
	adminService.SetChannelCacheInvalidator(channelService)
	...
	usageHandler.SetSettingService(settingService)
	...
	scheduledTestRunnerService := service.ProvideScheduledTestRunnerService(scheduledTestPlanRepository, scheduledTestService, accountTestService, accountProbeService, rateLimitService, configConfig)
	scheduledTestHandler := admin.NewScheduledTestHandler(scheduledTestService)
	scheduledTestHandler.SetRunnerService(scheduledTestRunnerService)
	...
	tokenCostHandler := admin.NewTokenCostHandler(tokenCostService)
	paymentHandler := admin.NewPaymentHandler(paymentService, paymentConfigService)
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, newAPICheckinHandler, accountRepository, tokenCostHandler, paymentHandler, affiliateHandler)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	gatewayHandler := handler.NewGatewayHandler(gatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, openAIGatewayService, compositeRouteResolver)
	openAIGatewayHandler := handler.NewOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, configConfig, compositeRouteResolver)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, apiKeyProbeHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient)
	httpServer := server.ProvideHTTPServer(configConfig, engine)
	opsMetricsCollector := service.ProvideOpsMetricsCollector(opsRepository, settingRepository, accountRepository, concurrencyService, db, redisClient, httpUpstream, configConfig)
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, schedulerSnapshotService, upstreamBalanceService, tokenRefreshService, accountExpiryService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, pricingService, emailQueueService, billingCacheService, userPlatformQuotaUsageFlusher, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, openAIRequestSnapshotService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator)
