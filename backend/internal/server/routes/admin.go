...
func registerNewAPICheckinRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	checkin := admin.Group("/newapi-checkin")
	{
		...
		checkin.POST("/api-keys/sync", h.Admin.NewAPICheckin.SyncAPIKeys)
		...
		checkin.POST("/login-credentials", h.Admin.NewAPICheckin.SaveLoginCredentials)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/scheduling-pool/:id/test", h.Admin.Account.Test)
		...
		accounts.GET("/batch-test-runs/:run_id", h.Admin.Account.GetBatchTestRun)
		...
		accounts.POST("/:id/refresh-upstream-balance", h.Admin.Account.RefreshUpstreamBalance)
