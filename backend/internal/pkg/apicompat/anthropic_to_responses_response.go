...
func AnthropicToResponsesResponse(resp *AnthropicResponse) *ResponsesResponse {
	...
	// Usage
	out.Usage = responsesUsageFromAnthropicUsage(resp.Usage)
...
type AnthropicEventToResponsesState struct {
	...
	// 当前输出项的完整内容会在关闭时写入 Outputs，供终态事件直接返回。
...
// anthropicResponsesStreamTerminalState 将 Anthropic 停止原因统一映射为 Responses 终态。
