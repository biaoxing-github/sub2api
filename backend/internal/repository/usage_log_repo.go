...
// Composite 只是路由层，统计必须归属最终被调度账号的具体平台。
...
// appendUsageLogUpstreamModelMismatchWhereCondition adds the optional audit filter.
func appendUsageLogUpstreamModelMismatchWhereCondition(conditions []string, args []any, mismatch *bool) ([]string, []any) {
	if mismatch == nil {
		return conditions, args
	}
	conditions = append(conditions, fmt.Sprintf("upstream_model_mismatch = $%d", len(args)+1))
	return conditions, append(args, *mismatch)
}
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	latencyStages := nullUsageLatencyStagesJSON(log.LatencyStages)
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendUsageLogUpstreamModelMismatchWhereCondition(conditions, args, filters.UpstreamModelMismatch)
...
func (r *usageLogRepository) GetUserBreakdownStats(ctx context.Context, startTime, endTime time.Time, dim usagestats.UserBreakdownDimension, limit int) (results []usagestats.UserBreakdownItem, err error) {
	...
	query += " GROUP BY ul.user_id, u.email ORDER BY " + resolveUserBreakdownOrderBy(dim.SortBy) + " DESC"
	if limit > 0 {
...
// resolveUserBreakdownOrderBy 将外部排序字段映射为固定 SQL 列名。
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		LatencyStages:         usageLatencyStagesFromNullJSON(latencyStages),
		CreatedAt:             createdAt,
	}
...
// nullUsageLatencyStagesJSON 将阶段耗时编码为可写入 jsonb 的参数。
func nullUsageLatencyStagesJSON(v *service.UsageLatencyStages) any {
	if v == nil {
		return nil
	}
	payload, err := json.Marshal(v)
...
// usageLatencyStagesFromNullJSON 将数据库 jsonb 还原为阶段耗时快照。
func usageLatencyStagesFromNullJSON(v sql.NullString) *service.UsageLatencyStages {
	if !v.Valid || strings.TrimSpace(v.String) == "" {
		return nil
	}
	var stages service.UsageLatencyStages
	if err := json.Unmarshal([]byte(v.String), &stages); err != nil {
		return nil
	}
