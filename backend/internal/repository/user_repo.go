...
// GetByIDIncludeDeleted 读取用户详情并显式包含 soft-deleted 记录。
...
// ApplyRedeemBalanceAdjustment 在当前事务内原子调整兑换余额，并保证余额不低于零。
...
// ApplyRedeemConcurrencyAdjustment 在当前事务内原子调整兑换并发，并保证并发不低于零。
