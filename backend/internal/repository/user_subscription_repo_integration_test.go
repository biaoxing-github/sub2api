...
func (s *UserSubscriptionRepoSuite) TestGetByIDIncludeDeletedPreservesPersistedStatus() {
...
func (s *UserSubscriptionRepoSuite) TestRestore_RequiresRevokedSubscription() {
	user := s.mustCreateUser("restore-not-revoked@test.com", service.RoleUser)
	group := s.mustCreateGroup("g-restore-not-revoked")
	sub := s.mustCreateSubscription(user.ID, group.ID, nil)
...
func (s *UserSubscriptionRepoSuite) TestList_IncludesRevokedWhenStatusEmpty() {
	...
	s.Require().NotNil(gotRevoked, "all status should include soft-deleted subscriptions")
...
func (s *UserSubscriptionRepoSuite) TestList_FilterByRevokedStatus() {
	...
	subs, page, err := s.repo.List(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, nil, nil, service.SubscriptionStatusRevoked, "", "", "")
	s.Require().NoError(err)
	s.Require().Len(subs, 1)
	s.Require().Equal(int64(1), page.Total)
...
func (s *UserSubscriptionRepoSuite) TestResetDailyUsage_MissingSubscriptionReturnsNotFound() {
	expectedWindowStart := time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)
	err := s.repo.ResetDailyUsage(s.ctx, 999999, &expectedWindowStart, expectedWindowStart.Add(24*time.Hour))
...
func (s *UserSubscriptionRepoSuite) TestActiveExpiredBoundaries_UsageAndReset_BatchUpdateExpiredStatus() {
	...
	s.Require().NoError(s.repo.ResetDailyUsage(s.ctx, active.ID, active.DailyWindowStart, resetAt), "ResetDailyUsage")
