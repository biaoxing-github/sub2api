...
// newRedeemAdjustmentRepoMock 创建仅覆盖兑换原子扣减 SQL 的仓储测试替身。
...
func TestApplyRedeemBalanceAdjustmentUsesAtomicFloor(t *testing.T) {
...
func TestApplyRedeemConcurrencyAdjustmentUsesAtomicFloor(t *testing.T) {
	repo, mock := newRedeemAdjustmentRepoMock(t)
...
func TestApplyRedeemAdjustmentReturnsNotFoundForMissingUser(t *testing.T) {
	repo, mock := newRedeemAdjustmentRepoMock(t)
	mock.ExpectExec(`UPDATE users SET balance = GREATEST\(balance \+ \$1, 0\), updated_at = NOW\(\) WHERE id = \$2 AND deleted_at IS NULL`).
