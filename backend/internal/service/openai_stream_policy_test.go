...
func TestOpenAIUpstreamErrorPolicySeparatesPhasesAndActions(t *testing.T) {
	cases := []struct {
		...
		{
			name:       "http infrastructure failure avoids account ttl",
			phase:      openAIUpstreamErrorPolicyPhaseHTTPResponse,
			input:      UpstreamErrorInput{StatusCode: http.StatusBadRequest, Body: []byte(`{"error":{"message":"disk storage creation failed: no space left on device"}}`)},
			wantAction: OpenAIStreamActionAvoidAccountTTL,
			wantScope:  "account",
			wantRecord: true,
		},
		{
			name:       "http 503 avoids account ttl",
			phase:      openAIUpstreamErrorPolicyPhaseHTTPResponse,
			input:      UpstreamErrorInput{StatusCode: http.StatusServiceUnavailable, Message: "service unavailable"},
			wantAction: OpenAIStreamActionAvoidAccountTTL,
			wantScope:  "account",
			wantRecord: true,
		},
		{
			name:       "http 504 also avoids account ttl",
			phase:      openAIUpstreamErrorPolicyPhaseHTTPResponse,
			input:      UpstreamErrorInput{StatusCode: http.StatusGatewayTimeout, Message: "gateway timeout"},
			wantAction: OpenAIStreamActionAvoidAccountTTL,
			wantScope:  "account",
			wantRecord: true,
		},
