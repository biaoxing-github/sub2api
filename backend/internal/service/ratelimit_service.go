...
func (s *RateLimitService) CheckErrorPolicy(ctx context.Context, account *Account, statusCode int, responseBody []byte, requestedModel ...string) ErrorPolicyResult {
	...
	if account.IsPoolMode() {
		// 池模式只跳过默认账号状态处理；管理员显式配置的非 401 临时规则仍应生效。
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	// 池模式默认不标记本地账号状态；管理员显式配置的非 401 临时规则仍优先。
	...
	case 401:
		...
		if account.Type == AccountTypeOAuth {
			...
			// 注意：这里不能回写 account.Credentials/expires_at。account 是请求开始时的快照，
			// 如果另一个 worker 刚刷新出新 refresh_token，整列覆盖 credentials 会把 DB 回滚到旧 token。
			// 冷却结束后由 token_provider / token_refresh_service 通过带锁刷新路径重新获取可用 token。
	...
	case 403:
		...
		if account.Platform == PlatformOpenAI && IsOpenAIImageGenerationNotEnabledError(statusCode, upstreamMsg, responseBody) {
			slog.Info("openai_image_generation_permission_skipped", "account_id", account.ID, "status_code", statusCode)
			shouldDisable = false
			break
		}
		logger.LegacyPrintf(
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	// 代理或 CDN 在请求到达 OpenAI API 前拦截时会返回 HTML 403。
	// 这只能证明当前链路或端点被拦截，不能据此处罚账号；请求仍保持原有换号语义。
...
// isHTMLResponse 识别代理或 CDN 返回的 HTML 错误页。
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte, requestedModel ...string) {
	...
	if resetTimestamp == "" {
		...
		// Anthropic 平台：没有限流重置时间的 429 不适合按窗口长时间封禁，
		// 但完全跳过会让调度器持续命中同一批账号，因此改走秒级兜底冷却。
		if account.Platform == PlatformAnthropic {
...
func (s *RateLimitService) applyUnifiedTempUnschedulable(ctx context.Context, account *Account, rule AccountErrorHandlingRule, statusCode int, responseBody []byte) bool {
	...
	modelKey := tempUnschedulableModel(ctx, nil)
	if modelKey != "" && statusCode != http.StatusUnauthorized {
		if err := s.accountRepo.SetModelRateLimit(ctx, account.ID, modelKey, until); err != nil {
			slog.Warn("unified_temp_unsched_model_rate_limit_set_failed", "account_id", account.ID, "model", modelKey, "status_code", statusCode, "rule", rule.Name, "error", err)
			return true
		}
		slog.Info("account_error_handling_model_temp_unschedulable", "account_id", account.ID, "model", modelKey, "status_code", statusCode, "rule", rule.Name, "until", until)
		return true
	}
...
// withTempUnschedulableModel 将当前调度模型带入嵌套错误策略，避免调用链丢失模型范围。
...
func (s *RateLimitService) triggerTempUnschedulable(ctx context.Context, account *Account, rule TempUnschedulableRule, ruleIndex int, statusCode int, matchedKeyword string, responseBody []byte, requestedModel ...string) bool {
	...
	// 已知模型的非认证错误只冷却当前账号与模型组合；写入失败也不扩大为整账号封锁。
