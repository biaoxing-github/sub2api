...
type blockingHTTPUpstreamRecorder struct {
	started chan struct{}
	err     error
}

func (u *blockingHTTPUpstreamRecorder) Do(req *http.Request, proxyURL string, accountID int64, accountConcurrency int) (*http.Response, error) {
	close(u.started)
	<-req.Context().Done()
	if u.err != nil {
		return nil, u.err
...
func (u *blockingHTTPUpstreamRecorder) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error) {
	return u.Do(req, proxyURL, accountID, accountConcurrency)
}
...
func TestOpenAIGatewayService_APIKeyPassthrough_UpstreamRequestPropagatesClientCancel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	account := &Account{
		...
		Concurrency:    1,
		Credentials:    map[string]any{"api_key": "test-api-key", "base_url": "https://api.example.com/v1"},
		Extra:          map[string]any{"openai_passthrough": true},
		Status:         StatusActive,
		Schedulable:    true,
		RateMultiplier: f64p(1),
	...
	result, err := svc.Forward(reqCtx, c, account, body)
	require.NoError(t, err)
	require.NotNil(t, result)
	require.NotNil(t, upstream.lastReq)
	require.ErrorIs(t, upstream.lastReq.Context().Err(), context.Canceled)
...
func TestOpenAIGatewayService_OAuthPassthroughUnexpectedEOFReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	originalBody := []byte(`{"model":"gpt-5.5","stream":false,"instructions":"keep going","input":[{"type":"text","text":"hi"}]}`)

	upstream := &httpUpstreamRecorder{
		err: errors.New("Post \"https://chatgpt.com/backend-api/codex/responses\": unexpected EOF"),
	}
	svc := &OpenAIGatewayService{
		cfg:          &config.Config{},
		httpUpstream: upstream,
...
func TestOpenAIGatewayService_OAuthPassthroughHeaderWaitTimeoutReturnsFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &blockingHTTPUpstreamRecorder{started: make(chan struct{})}
	svc := &OpenAIGatewayService{
		cfg: &config.Config{Gateway: config.GatewayConfig{
			OpenAIRequestHeaderTimeoutSeconds: 1,
		}},
		httpUpstream: upstream,
	}
	account := &Account{
...
func TestOpenAIGatewayService_OAuthPassthrough_Non2xxTriggersFailoverWithPassthroughFlag(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.ErrorAs(t, err, &failoverErr)
	require.Equal(t, http.StatusBadRequest, failoverErr.StatusCode)
	require.False(t, c.Writer.Written(), "所有非 2xx passthrough 响应都应返回 failover 错误给上层换号")
