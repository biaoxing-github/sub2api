...
type SubscriptionService struct {
	...
	// subCacheInvalidationCancel 在服务停止时关闭跨实例 L1 缓存失效订阅。
...
func NewSubscriptionService(groupRepo GroupRepository, userSubRepo UserSubscriptionRepository, billingCacheService *BillingCacheService, entClient *dbent.Client, cfg *config.Config) *SubscriptionService {
	...
	subCacheInvalidationCtx, cancelSubCacheInvalidation := context.WithCancel(context.Background())
	svc.subCacheInvalidationCancel = cancelSubCacheInvalidation
	svc.StartSubCacheInvalidationSubscriber(subCacheInvalidationCtx)
	return svc
}
...
// InvalidateSubCacheSync 失效订阅 L1 缓存并等待删除可见，避免维护后立刻读到旧快照。
...
// invalidateSubCacheKeySync 按缓存 key 同步失效本实例的订阅 L1。
func (s *SubscriptionService) invalidateSubCacheKeySync(cacheKey string) {
	if s.subCacheL1 == nil {
		return
	}
	s.subCacheL1.Del(cacheKey)
	s.subCacheL1.Wait()
}
...
func (s *SubscriptionService) StartSubCacheInvalidationSubscriber(ctx context.Context) {
	...
	if err := s.billingCacheService.SubscribeSubscriptionCacheInvalidation(ctx, s.invalidateSubCacheKeySync); err != nil {
		log.Printf("Warning: failed to start subscription cache invalidation subscriber: %v", err)
	}
}

// invalidateSubscriptionCaches 在订阅写入完成后同步失效本地、共享和其他实例的缓存。
...
// assignOrExtendSubscription 在支付事务中可延后缓存失效，避免未提交数据回填缓存。
func (s *SubscriptionService) assignOrExtendSubscription(ctx context.Context, input *AssignSubscriptionInput, deferCacheInvalidation bool) (*UserSubscription, bool, error) {
	...
	if existingSub != nil {
		...
		if err := s.maybeInvalidateAssignmentCaches(input.UserID, input.GroupID, deferCacheInvalidation); err != nil {
			return nil, false, err
		}
...
// maybeInvalidateAssignmentCaches 由支付履约事务在提交后统一失效缓存。
func (s *SubscriptionService) maybeInvalidateAssignmentCaches(userID, groupID int64, deferred bool) error {
	if deferred {
		return nil
	}
...
func (s *SubscriptionService) assignSubscriptionWithReuse(ctx context.Context, input *AssignSubscriptionInput) (*UserSubscription, bool, error) {
	...
	if exists {
		...
		if sub.Status == SubscriptionStatusExpired ||
			...
			if err := s.maybeInvalidateAssignmentCaches(input.UserID, input.GroupID, false); err != nil {
				return nil, false, err
			}
...
// RestoreSubscription 恢复已撤销订阅；若原订阅已过期，则恢复为过期状态。
...
// EnsureWindowMaintenance 在请求放行前同步推进窗口，并回读数据库以消除 CAS 竞争中的旧内存快照。
