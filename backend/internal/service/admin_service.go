...
type AdminService interface {
	...
	// Account management
	ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode, planType string, sortBy, sortOrder string) ([]Account, int64, error)
	...
	RecoverAccountAfterManualProbe(ctx context.Context, accountID int64, platform string) (*Account, error)
...
type CreateUserInput struct {
	...
	Username      string
	Notes         string
	Role          string // 空字符串表示使用默认角色 user；合法值为 admin/user。
	Balance       float64
	Concurrency   int
	RPMLimit      int
	AllowedGroups []int64
	ActorAdminID  int64 // 执行操作的管理员 ID，用于权限变更审计。
}
...
type UpdateUserInput struct {
	...
	Username      *string
	Notes         *string
	Role          string   // 空字符串表示不修改；合法值为 admin/user。
	...
	// ActorAdminID 是执行操作的管理员 ID，用于自降级保护和权限变更审计。
	ActorAdminID int64
}
...
type groupModelsListProvider interface {
...
// SetGatewayService 注入分组模型候选所需的网关模型读取器。
func (s *adminServiceImpl) SetGatewayService(gatewayService groupModelsListProvider) {
	if s == nil {
		return
	}
...
// normalizeUserRole 校验并归一化角色；空字符串使用 fallback。
...
// ensureNotLastAdmin 确认降级目标之外仍有管理员，避免后台失去可登录管理员。
func (s *adminServiceImpl) ensureNotLastAdmin(ctx context.Context) error {
	...
	_, result, err := s.userRepo.ListWithFilters(
		...
		UserListFilters{Role: RoleAdmin, IncludeSubscriptions: &includeSubscriptions},
	)
	if err != nil {
...
func (s *adminServiceImpl) UpdateUser(ctx context.Context, id int64, input *UpdateUserInput) (*User, error) {
	...
	// 只写管理员本次明确提交的列，避免旧快照覆盖并发扣费或状态变更。
	...
	if input.Role != "" {
		...
		if user.Role == RoleAdmin && role == RoleUser {
			if input.ActorAdminID != 0 && input.ActorAdminID == id {
				return nil, errors.New("cannot demote yourself from admin")
			}
	...
	if user.Role != oldRole {
		logger.LegacyPrintf(
			"service.admin",
			"audit: user role changed actor_admin_id=%d target_user_id=%d old_role=%s new_role=%s",
			input.ActorAdminID,
			user.ID,
			oldRole,
			user.Role,
...
func (s *adminServiceImpl) GetGroupModelsListCandidates(ctx context.Context, id int64, platform string) ([]string, error) {
	...
	if s.gatewayService != nil {
		extraModels = s.gatewayService.GetAvailableModels(ctx, &id, platform)
...
func (s *adminServiceImpl) UpdateGroup(ctx context.Context, id int64, input *UpdateGroupInput) (*Group, error) {
	...
	// 更新限额时 nil 保留原值，负数清除限制，0 禁止用量，正数设置限额。
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if len(input.Credentials) > 0 {
		...
		account.Credentials = MergeAccountCredentialsForUpdate(account.Credentials, input.Credentials)
	}
...
func (s *adminServiceImpl) CreateProxy(ctx context.Context, input *CreateProxyInput) (*Proxy, error) {
	mode, err := normalizeAdminProxyFallbackMode(input.FallbackMode)
	if err != nil {
		return nil, err
	}
...
func normalizeAdminProxyFallbackMode(mode string) (string, error) {
	...
	default:
		return "", infraerrors.BadRequest("PROXY_FALLBACK_MODE_INVALID", "fallback_mode must be one of none, proxy, direct")
	}
}
