...
// paymentFulfillmentLease 绑定履约工作者本次抢占到的订单版本。
...
// acquirePaymentFulfillmentLease 原子抢占待履约订单，并允许恢复超过五分钟的陈旧租约。
...
// ensurePaymentSubscriptionAssigned 在履约事务内写入订阅和可恢复审计记录。
...
// affiliateRebateBaseAmount 返回可产生邀请返利的订单金额。
...
// buildAffiliateRebateAuditClaimQuery 根据驱动方言生成幂等返利审计占位语句。
