...
func TestAPIKeyUpdate_OnlyDeclaresRequestedColumns(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want: APIKeyUpdateFields{Name: true, IPRules: true},
		},
		{
			name: "quota only",
			req:  UpdateAPIKeyRequest{Quota: &quota},
			want: APIKeyUpdateFields{Quota: true, IPRules: true},
		},
		{
			name: "rate limit threshold only",
			req:  UpdateAPIKeyRequest{RateLimit5h: &rateLimit},
			want: APIKeyUpdateFields{RateLimits: true, IPRules: true},
		},
		{
			name: "ip whitelist only",
			req:  UpdateAPIKeyRequest{IPWhitelist: []string{"10.0.0.1"}},
...
func TestAPIKeyUpdate_DeclaresUsageColumnsOnExplicitReset(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []APIKeyUpdateFields{{QuotaUsed: true, RateLimitUsage: true, IPRules: true}}, repo.updateFields)
}
...
func TestAPIKeyUpdate_DeclaresStatusWhenReactivated(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, []APIKeyUpdateFields{{Quota: true, Status: true, IPRules: true}}, repo.updateFields)
}
