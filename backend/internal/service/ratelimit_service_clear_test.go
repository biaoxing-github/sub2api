...
func TestRateLimitService_RecoverAccountState_ClearsErrorRateLimitAndRestoresSchedulableWhenRequested(t *testing.T) {
	now := time.Now()
	repo := &rateLimitClearRepoStub{
		getByIDAccount: &Account{
			ID:                      42,
	...
	result, err := svc.RecoverAccountState(context.Background(), 42, AccountRecoveryOptions{
