...
func currentCodexOriginatorForTest() string {
	originator, _, ok := openai.PairCodexClientIdentity(codexCLIUserAgent())
	if !ok {
		return "codex_cli_rs"
	}
	return originator
...
func TestEnsureCodexIdentityHeaders(t *testing.T) {
	t.Run("补齐缺失身份头", func(t *testing.T) {
		...
		require.Equal(t, currentCodexOriginatorForTest(), h.Get("originator"))
