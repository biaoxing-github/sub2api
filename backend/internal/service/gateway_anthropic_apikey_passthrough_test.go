...
func TestGatewayService_AnthropicAPIKeyPassthrough_FormatContentIssueTriggersFailover(t *testing.T) {
	...
	body := []byte(`{"model":"claude-opus-4-8","stream":false,"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`)
	parsed := &ParsedRequest{
		Body:   NewRequestBodyRef(body),
	...
	upstreamBody := `{"error":{"type":"<nil>","message":"There was an issue with the format or content of your request. (request id: 202606100732187175600488268d9d6syj42KHv)"},"type":"error"}`
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusBadRequest,
			Header: http.Header{
				"Content-Type": []string{"application/json"},
				"x-request-id": []string{"rid-format-content"},
			},
	...
	require.Nil(t, result)
	var failoverErr *UpstreamFailoverError
	require.True(t, errors.As(err, &failoverErr), "Anthropic API Key 透传格式错误应交给 handler 切账号，而不是直接写回客户端")
	require.Equal(t, http.StatusBadRequest, failoverErr.StatusCode)
	require.Contains(t, string(failoverErr.ResponseBody), "202606100732187175600488268d9d6syj42KHv")
	require.Equal(t, 0, rec.Body.Len(), "failover 前不能向客户端写入 fallback error")
}

func TestGatewayService_AnthropicAPIKey_FormatContentIssueTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	svc := &GatewayService{
		...
		httpUpstream:     upstream,
		rateLimitService: &RateLimitService{},
		settingService:   NewSettingService(&settingRepoStub{}, &config.Config{}),
	}
	account := newAnthropicAPIKeyAccountForTest()
	account.Extra = nil
	...
	require.Nil(t, result)
	var failoverErr *UpstreamFailoverError
	require.True(t, errors.As(err, &failoverErr), "Anthropic API Key 原生路径格式错误应交给 handler 切账号")
	require.Equal(t, http.StatusBadRequest, failoverErr.StatusCode)
	require.Contains(t, string(failoverErr.ResponseBody), "202606100732187175600488268d9d6syj42KHv")
	require.Equal(t, 0, rec.Body.Len(), "failover 前不能向客户端写入 fallback error")
}

func TestGatewayService_AnthropicAPIKeyPassthrough_Ordinary400KeepsDefaultError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			...
			Body:       io.NopCloser(strings.NewReader(`{"error":{"message":"messages.0.content: field required"},"type":"error"}`)),
		},
	}
	...
	require.Nil(t, result)
	var failoverErr *UpstreamFailoverError
	require.False(t, errors.As(err, &failoverErr), "普通 400 不应误触发账号切换")
	require.Error(t, err)
	require.Equal(t, http.StatusBadRequest, rec.Code)
	require.Contains(t, rec.Body.String(), "messages.0.content")
}
...
func TestGatewayService_AnthropicOAuth_ForwardPreservesBillingHeaderSystemBlock(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                       "system array with cache breakpoint",
			body:                       `{"model":"claude-haiku-4-5","system":[{"type":"text","text":"Pi project instructions","cache_control":{"type":"ephemeral","ttl":"1h"}}],"messages":[{"role":"user","content":[{"type":"text","text":"hello"}]}]}`,
...
func TestGatewayService_AnthropicAPIKeyPassthrough_MissingTerminalBeforeOutputTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}},
		Body:       io.NopCloser(&apiKeyPassthroughEmptyReader{}),
	}

	result, err := svc.handleStreamingResponseAnthropicAPIKeyPassthrough(context.Background(), resp, c, &Account{ID: 1}, time.Now(), "GLM-5.1")
	require.Error(t, err)
	require.Nil(t, result)

	var failoverErr *UpstreamFailoverError
	require.True(t, errors.As(err, &failoverErr), "valid-looking passthrough streams without client output should trigger failover")
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.False(t, failoverErr.RetryableOnSameAccount)
	require.Contains(t, string(failoverErr.ResponseBody), `"stream_incomplete"`)
	require.Empty(t, rec.Body.String())
}

func TestGatewayService_AnthropicAPIKeyPassthrough_MissingTerminalAfterOutputSendsErrorEvent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Contains(t, body, "event: error\n", "partial passthrough streams that end without a terminal event must notify Anthropic clients")
	require.Contains(t, body, `"type":"error"`)
	require.Contains(t, body, `"stream_incomplete"`)
	require.Contains(t, body, "upstream stream ended before a terminal event")
}

func TestGatewayService_AnthropicAPIKeyPassthrough_DataErrorBeforeOutputTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	var failoverErr *UpstreamFailoverError
	require.True(t, errors.As(err, &failoverErr), "data-only passthrough error events before output must trigger failover")
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.False(t, failoverErr.RetryableOnSameAccount)
...
func TestGatewayService_AnthropicAPIKeyPassthrough_ZeroUsageTerminalBeforeOutputTriggersFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	resp := &http.Response{
		...
		Body: io.NopCloser(strings.NewReader(strings.Join([]string{
			`event: message_start`,
			`data: {"type":"message_start","message":{"id":"msg_zero","type":"message","role":"assistant","content":[],"usage":{"input_tokens":0,"output_tokens":0}}}`,
			"",
	...
	var failoverErr *UpstreamFailoverError
	require.True(t, errors.As(err, &failoverErr), "zero-token terminal streams should fail over before reaching Hermes")
	require.Equal(t, http.StatusBadGateway, failoverErr.StatusCode)
	require.False(t, failoverErr.RetryableOnSameAccount)
	require.Contains(t, string(failoverErr.ResponseBody), `"stream_invalid"`)
	require.Empty(t, rec.Body.String())
}
