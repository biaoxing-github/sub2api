	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		for baseURLIndex, requestBaseURL := range baseURLsForAttempt {
			...
			if resp.StatusCode != http.StatusOK {
				...
				if !accountScheduled && resp.StatusCode == http.StatusUnauthorized && s.accountRepo != nil {
...
func createGeminiTestPayload(modelID string, prompt string) []byte {
	...
	if textPrompt == "" {
		textPrompt = RandomQuickValidationPrompt()
	}
...
func (s *AccountTestService) testOpenAIImageAPIKey(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	if resp.StatusCode != http.StatusOK {
		s.scheduleOpenAIAPIKeyFromTestError(ctx, account, resp.StatusCode, body)
		return s.sendErrorAndEnd(c, fmt.Sprintf("API returned %d: %s", resp.StatusCode, string(body)))
	}
...
func (s *AccountTestService) testOpenAIImageOAuth(c *gin.Context, ctx context.Context, account *Account, modelID, prompt string) error {
	...
	req, err := s.buildOpenAITestResponsesRequest(ctx, c, account, responsesBody, authToken, true, "", "", "/v1/responses")
	if err != nil {
		return s.sendErrorAndEnd(c, "Failed to create request")
	}
