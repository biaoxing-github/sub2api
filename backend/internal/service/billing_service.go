...
// UserPlatformQuotaCacheSchemaV1 是当前 user×platform quota 缓存的 schema 版本。
const UserPlatformQuotaCacheSchemaV1 = int64(1)

// UserPlatformQuotaCacheEntry 表示 Redis 中的 user×platform quota 快照。
// SchemaVersion 为 0 时视为旧缓存，调用方应当当作 MISS 回源刷新。
// Limit 字段为 nil 表示无限额，对应 DB 中的 NULL。
...
type ModelPricing struct {
	...
	CacheCreationPriceExplicit         bool    // 渠道或区间显式设置缓存创建价格，即使为零也不回退
...
// CalculateWebSearchCost 计算 alpha/search 网页搜索按次费用。
...
func (s *BillingService) initFallbackPricing() {
	...
	// Claude 4.8 与 Opus 5 官方同价；显式登记可避免回退到旧 Opus 的 $15/$75。
...
func (s *BillingService) GetModelPricingWithChannel(model string, channelPricing *ChannelModelPricing) (*ModelPricing, error) {
	...
	// 渠道价格覆盖只影响本次解析，不能写回 fallbackPrices 中共享的模型指针。
	cloned := *pricing
	pricing = &cloned
...
func (s *BillingService) computeTokenBreakdown(
	...
	if applyLongCtx && s.shouldApplySessionLongContextPricing(tokens, pricing) {
		...
		// cache_read / cache_write 都是输入侧计费项，长上下文整次会话涨价时必须同步放大。
...
// normalizeCacheCreationBreakdown 在明细超过总量时按比例限制缓存 token，避免重复收费。
...
func (s *BillingService) computeCacheCreationCost(pricing *ModelPricing, tokens UsageTokens, multiplier, unitPrice float64) float64 {
	...
	return float64(tokens.CacheCreationTokens) * unitPrice * multiplier
}
