...
func TestForwardAsRawChatCompletions_UsesCodexSimulationHeaders(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstreamBody := strings.Join([]string{
		`data: {"id":"chatcmpl_codex","object":"chat.completion.chunk","model":"gpt-5.5","choices":[{"index":0,"delta":{"content":"ok"}}]}`,
		"",
		"data: [DONE]",
		"",
	}, "\n")
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"text/event-stream"}, "x-request-id": []string{"rid_raw_codex_headers"}},
		Body:       io.NopCloser(strings.NewReader(upstreamBody)),
	}}
	...
	account := rawChatCompletionsTestAccount()
	account.Extra = map[string]any{OpenAICodexCLISimulationEnabledExtraKey: true}
