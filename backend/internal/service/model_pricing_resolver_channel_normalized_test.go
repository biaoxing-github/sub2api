...
const (
	...
	// 本地 GPT-5.6 在 1M 上下文时按既有策略计算两倍输入费用。
	channelPricingLongContextMultiplier = 2.0
)
...
func TestChannelPricing_ExactModelMatch(t *testing.T) {
	...
	require.InDelta(t, channelPricingExpectedChannelCost*channelPricingLongContextMultiplier, log.InputCost, 1e-9)
}
...
func TestChannelPricing_ExactVariantWinsOverNormalizedBaseName(t *testing.T) {
	...
	require.InDelta(t, channelPricingUnrelatedCost*channelPricingLongContextMultiplier, log.InputCost, 1e-9,
...
func TestChannelPricing_UnrelatedChannelModelNotMatched(t *testing.T) {
	...
	require.InDelta(t, channelPricingExpectedOfficialCost*channelPricingLongContextMultiplier, log.InputCost, 1e-9,
