...
const (
	...
	schedulerOutboxCleanBatch = 5000
)
...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	useMixed := schedulerSnapshotUsesMixedPlatforms(platform, hasForcePlatform)
...
func (s *SchedulerSnapshotService) cleanupConsumedOutbox(watermark int64) {
	...
	for {
		deleted, err := s.outboxRepo.DeleteConsumedUpTo(ctx, watermark, schedulerOutboxCleanBatch)
		if err != nil {
			logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] outbox cleanup failed: watermark=%d err=%v", watermark, err)
			return
		}
		if deleted == 0 || deleted < schedulerOutboxCleanBatch {
			return
		}
	}
}
...
func (s *SchedulerSnapshotService) rebuildBucketsForPlatform(ctx context.Context, platform string, groupIDs []int64, reason string, seen map[batchSeenKey]struct{}) error {
	...
	for _, gid := range groupIDs {
		...
		if schedulerSnapshotUsesMixedPlatforms(platform, false) {
...
func (s *SchedulerSnapshotService) loadAccountsFromDB(ctx context.Context, bucket SchedulerBucket, useMixed bool) ([]Account, error) {
	...
	if useMixed {
		platforms := schedulerSnapshotMixedPlatforms(bucket.Platform)
...
func schedulerSnapshotUsesMixedPlatforms(platform string, hasForcePlatform bool) bool {
	if hasForcePlatform {
		return false
	}
...
func schedulerSnapshotMixedPlatforms(platform string) []string {
	if platform == PlatformOpenAI {
		return openAICompatibleAccountPlatforms(platform)
	}
...
func (s *SchedulerSnapshotService) defaultBuckets(ctx context.Context) ([]SchedulerBucket, error) {
	...
	for _, group := range groups {
		...
		if schedulerSnapshotUsesMixedPlatforms(group.Platform, false) {
