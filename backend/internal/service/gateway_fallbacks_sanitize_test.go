...
func TestBuildUpstreamRequest_OAuthMimicHaiku_StripsFallbacksEndToEnd(t *testing.T) {
	...
	baseline, _, err := svc.buildUpstreamRequest(context.Background(), c, account,
		[]byte(`{"model":"claude-haiku-4-5","messages":[]}`),
		"oauth-tok", "oauth", "claude-haiku-4-5", false, true)
	require.NoError(t, err)
	require.Equal(t, getHeaderRaw(baseline.Header, "anthropic-beta"), outBeta,
		"保留本地 Haiku 的 beta 过滤规则")
}
...
func TestPrepareBedrockRequestBodyWithTokens_FallbacksRequireSupportedBeta(t *testing.T) {
	...
	t.Run("strips fallbacks even when client passes server-side-fallback token (not whitelisted)", func(t *testing.T) {
		...
		result, err := PrepareBedrockRequestBodyWithTokens(
			[]byte(input), modelID, filterBedrockBetaTokens([]string{claude.BetaServerSideFallback}), false,
		)
		require.NoError(t, err)

		assert.False(t, gjson.GetBytes(result, "fallbacks").Exists(),
			"Bedrock 白名单不含 fallback beta → 条件 strip 总会剥除（预期）")
		for _, name := range gjson.GetBytes(result, "anthropic_beta").Array() {
			assert.NotEqual(t, claude.BetaServerSideFallback, name.String(),
