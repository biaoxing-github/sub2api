...
// TestForwardAsAnthropic_ForceChatCompletionsPreservesFinalModelReasoningEffort
// 验证 Chat Completions 回退桥接依据最终映射模型保留或归一化推理强度。
func TestForwardAsAnthropic_ForceChatCompletionsPreservesFinalModelReasoningEffort(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "old model max", model: "gpt-5.5", mapped: "gpt-5.5", effortJSON: `,"output_config":{"effort":"max"}`, wantEffort: "xhigh"},
		{name: "high remains high", model: "gpt-5.6-luna", mapped: "gpt-5.6-luna", effortJSON: `,"output_config":{"effort":"high"}`, wantEffort: "high"},
		{name: "omitted defaults medium", model: "gpt-5.6-luna", mapped: "gpt-5.6-luna", wantEffort: "medium"},
	}
