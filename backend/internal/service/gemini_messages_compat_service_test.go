...
// TestGeminiForwardAsChatCompletions_FunctionNamedWebSearchStaysClientSide 验证
// Hermes 普通 function 工具不会仅因名称为 web_search 而被提升为服务端搜索工具。
func TestGeminiForwardAsChatCompletions_FunctionNamedWebSearchStaysClientSide(t *testing.T) {
	...
	require.True(t, ok)
	require.Len(t, tools, 1, "Chat Completions function 工具不能只按名称提升为 Gemini 内置工具")
