...
// ModelPlazaRuntime 是模型广场公开接口使用的轻量运行时配置。
...
// GetModelPlazaRuntime 直接读取模型广场配置；读取失败时按默认关闭处理。
...
// OmittedSettingKeys 标记请求 payload 中没有出现的 setting key。
// nil 或空集合保持原有全量写入语义。
type OmittedSettingKeys map[string]struct{}

// dropFrom 从待写入集合中删除请求未携带的 key。
...
// UpdateSettingsOmitting 更新系统设置，同时保留 omitted 中列出的历史值。
...
// UpdateSettingsWithAuthSourceDefaultsOmitting 在同一批次更新系统设置与认证源默认值，
// 同时保留 omitted 中列出的历史 setting key。
...
// refreshCachedSettingsAfterWrite 保证运行时缓存与最终持久化值一致。
// 部分 payload 的请求结构含有大量零值，因此需要从存储重新读取后刷新缓存。
...
// PasskeyEnabled 返回部署配置与数据库开关共同决定的运行时状态。
// 数据库只允许关闭有效的 RP 配置，不能替代部署层的 WebAuthn 信任边界。
...
// PasskeyConfiguration 返回管理端可展示的非敏感 RP 配置。
func (s *SettingService) PasskeyConfiguration() (configured bool, rpID string, origins []string) {
	...
	return s.cfg.WebAuthn.Enabled,
		...
		append([]string{}, s.cfg.WebAuthn.RPOrigins...)
}
...
// passkeySettingEnabled 保证移除部署配置后，残留的数据库 true 不会误报为可用。
