...
const (
	...
	// backupScheduledLeaderLockKey 标识定时全库备份的集群选主锁。
	backupScheduledLeaderLockKey = "backup:scheduled:leader"
	// backupScheduledLeaderLockTTL 必须长于定时备份的 30 分钟执行上限，避免备份过程中锁过期。
...
type BackupService struct {
	...
	lockCache  LeaderLockCache // 定时备份优先使用的分布式选主锁后端
	db         *sql.DB         // 缓存锁不可用时使用的数据库 advisory lock 后端
	instanceID string          // 当前服务实例持有选主锁时使用的 owner 标识
...
// SetLeaderLock 注入定时备份选主所需的缓存和数据库锁后端；两者均为空时保持单实例直接执行行为。
...
func (s *BackupService) runScheduledBackup() {
	...
	// 仅约束 cron 触发的备份，手动 CreateBackup/StartBackup 仍可由运维在任意实例触发。
