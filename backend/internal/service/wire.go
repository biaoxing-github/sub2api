...
func ProvideUpdateService(cache UpdateCache, githubClient GitHubReleaseClient, buildInfo BuildInfo) *UpdateService {
	return NewUpdateServiceWithImageVersion(cache, githubClient, buildInfo.Version, buildInfo.ImageVersion, buildInfo.BuildType)
}
...
func ProvideTokenRefreshService(
	...
	svc := NewTokenRefreshService(accountRepo, oauthService, openaiOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, cacheInvalidator, schedulerCache, cfg, tempUnschedCache)
...
func ProvideProxyExpiryService(proxyRepo ProxyExpiryRepository) *ProxyExpiryService {
...
// ProvideUserPlatformQuotaUsageFlusher 创建并启动用户平台配额 DB 镜像刷写器。
func ProvideUserPlatformQuotaUsageFlusher(cfg *config.Config, cache BillingCache, quotaRepo UserPlatformQuotaRepository, timingWheel *TimingWheelService) *UserPlatformQuotaUsageFlusher {
	svc := NewUserPlatformQuotaUsageFlusher(cfg, cache, quotaRepo, timingWheel)
	svc.Start()
	return svc
}
...
func ProvideOpsMetricsCollector(
	...
	collector := NewOpsMetricsCollector(opsRepo, settingRepo, accountRepo, concurrencyService, db, redisClient, httpUpstream, cfg)
	collector.Start()
	return collector
}
...
func ProvideScheduledTestRunnerService(
	...
	accountProbeSvc *AccountProbeService,
	rateLimitSvc *RateLimitService,
	cfg *config.Config,
) *ScheduledTestRunnerService {
	svc := NewScheduledTestRunnerService(planRepo, scheduledSvc, accountTestSvc, rateLimitSvc, cfg)
	svc.SetAccountProbeRunner(accountProbeSvc)
	svc.Start()
	return svc
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) *SettingService {
	...
	if err := svc.LoadRuntimeSettings(context.Background()); err != nil {
		logger.LegacyPrintf("service.setting", "Warning: load runtime settings failed: %v", err)
	}
...
// ProvideAdminService 创建 AdminService 并注入网关模型列表读取器。
