...
// shouldStripOpenAIResponsesInputItemID 判断回放 ID 是否符合官方 Responses item 前缀约束。
// 非法 ID 直接移除，避免伪造的新 ID 错误引用其他上游对象。
...
// sanitizeOpenAIResponsesInputItemIDs 单次重建 input，清理 API Key 透传路径中的非法 item ID。
