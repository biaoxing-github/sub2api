...
// TestBackupService_RunScheduledBackup_LeaderElection 验证定时备份只由集群 leader 执行，并在完成后释放锁。
func TestBackupService_RunScheduledBackup_LeaderElection(t *testing.T) {
	t.Run("非 leader 跳过", func(t *testing.T) {
		repo := newMockSettingRepo()
		seedS3Config(t, repo)
		store := newMockObjectStore()
		...
		store.mu.Lock()
		require.Empty(t, store.objects, "非 leader 不应上传备份")
		store.mu.Unlock()

		records, err := svc.ListBackups(context.Background())
		require.NoError(t, err)
		require.Empty(t, records, "非 leader 不应创建备份记录")
		require.Equal(t, "peer", cache.heldBy(backupScheduledLeaderLockKey), "原 owner 应继续持有锁")
	})

	t.Run("leader 执行并释放锁", func(t *testing.T) {
		repo := newMockSettingRepo()
		seedS3Config(t, repo)
		store := newMockObjectStore()
		...
		records, err := svc.ListBackups(context.Background())
		require.NoError(t, err)
		require.Len(t, records, 1, "leader 应创建且仅创建一条备份记录")
		require.Equal(t, "completed", records[0].Status)
		require.Empty(t, cache.heldBy(backupScheduledLeaderLockKey), "leader 完成后应释放锁")
	})
}
