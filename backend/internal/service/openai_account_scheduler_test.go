...
func (r schedulerGroupAwareOpenAIAccountRepo) ListSchedulableByGroupIDAndPlatforms(ctx context.Context, groupID int64, platforms []string) ([]Account, error) {
	...
	for _, acc := range r.accounts {
		account := acc
		if schedulerTestPlatformIn(acc.Platform, platforms) && isAccountInRequestedGroup(&account, &groupID) {
...
func (r schedulerGroupAwareOpenAIAccountRepo) ListSchedulableUngroupedByPlatforms(ctx context.Context, platforms []string) ([]Account, error) {
	...
	for _, acc := range r.accounts {
		account := acc
		if schedulerTestPlatformIn(acc.Platform, platforms) && isAccountInRequestedGroup(&account, nil) {
			result = append(result, schedulerTestEnsureOpenAIAPIKeyCredentials(acc))
		}
	}
	return result, nil
...
func (r schedulerTestOpenAIAccountRepo) ListSchedulableByGroupIDAndPlatforms(ctx context.Context, groupID int64, platforms []string) ([]Account, error) {
	var result []Account
	for _, acc := range r.accounts {
		if schedulerTestPlatformIn(acc.Platform, platforms) {
			result = append(result, schedulerTestEnsureOpenAIAPIKeyCredentials(acc))
		}
	}
	return result, nil
...
func schedulerTestPlatformIn(platform string, platforms []string) bool {
...
func TestOpenAIGatewayService_LegacyLoadAwarenessPrefersRemainingConcurrencyCapacity(t *testing.T) {
	ctx := context.Background()
	groupID := int64(10107)
	accounts := []Account{
		{
	...
	loadCache := schedulerTestConcurrencyCache{
		loadMap: map[int64]*AccountLoadInfo{
			36003: {AccountID: 36003, CurrentConcurrency: 2, LoadRate: 20},
			36004: {AccountID: 36004, CurrentConcurrency: 0, LoadRate: 0},
		},
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerTestOpenAIAccountRepo{accounts: accounts},
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_PreviousResponseSkipsAccountOutsideGroup(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	requestGroupID := int64(10207)
	otherGroupID := int64(10208)
	accounts := []Account{
		{
			ID:            37101,
			Platform:      PlatformOpenAI,
		...
		},
		{
			...
			Concurrency:   1,
			Priority:      5,
			AccountGroups: []AccountGroup{{GroupID: requestGroupID}},
			Extra: map[string]any{
				"openai_apikey_responses_websockets_v2_enabled": true,
			},
		},
	...
	store := svc.getOpenAIWSStateStore()
	require.NoError(t, store.BindResponseAccount(ctx, requestGroupID, "resp_cross_group", 37101, time.Hour))
	...
	accountID, lookupErr := store.GetResponseAccount(ctx, requestGroupID, "resp_cross_group")
	require.NoError(t, lookupErr)
	require.Zero(t, accountID)
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_SessionStickySkipsAccountOutsideGroup(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	requestGroupID := int64(10217)
	otherGroupID := int64(10218)
	sessionHash := "session_cross_group"
	accounts := []Account{
		{
			ID:            37201,
			Platform:      PlatformOpenAI,
	...
	cfg.Gateway.Scheduling.LoadBatchEnabled = false
	cache := &schedulerTestGatewayCache{
		sessionBindings: map[string]int64{"openai:" + sessionHash: 37201},
	}
	svc := &OpenAIGatewayService{
		accountRepo:        schedulerGroupAwareOpenAIAccountRepo{schedulerTestOpenAIAccountRepo: schedulerTestOpenAIAccountRepo{accounts: accounts}},
		cache:              cache,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_LoadBalanceDoesNotPrecheckRealtimeBalance(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_AllowsOnlyAPIKeyWhenRealtimeBalanceUnknown(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	groupID := int64(101141)
	account := Account{
		ID:          38203,
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
	...
	}), RealtimeBalanceCheckerOptions{Timeout: time.Second, CandidateTopN: 3})
	svc := &OpenAIGatewayService{
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(38203), selection.Account.ID)
	require.Equal(t, openAIAccountScheduleLayerLoadBalance, decision.Layer)
}

func TestOpenAIGatewayService_SelectAccountWithScheduler_LoadBalanceSkipsTopNRealtimeBalance(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()

	ctx := context.Background()
	groupID := int64(101145)
	accounts := []Account{
		{ID: 38211, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 0},
		{ID: 38212, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 1},
		{ID: 38213, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Status: StatusActive, Schedulable: true, Concurrency: 1, Priority: 2},
	}
	cfg := &config.Config{}
	cfg.Gateway.Scheduling.LoadBatchEnabled = true
	checks := make(chan int64, 3)
	checker := NewRealtimeBalanceChecker(realtimeBalanceRefresherFunc(func(ctx context.Context, account *Account) (*UpstreamBalanceSnapshot, error) {
		checks <- account.ID
		return &UpstreamBalanceSnapshot{Available: 0, OKCount: 1}, nil
	}), RealtimeBalanceCheckerOptions{Timeout: time.Second, CandidateTopN: 3})
	...
	require.NotNil(t, selection)
	require.NotNil(t, selection.Account)
	require.Equal(t, int64(38211), selection.Account.ID)

	select {
	case id := <-checks:
		t.Fatalf("unexpected realtime balance check for account %d", id)
	default:
	}
}
