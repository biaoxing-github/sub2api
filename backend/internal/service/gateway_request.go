...
// OpenAIParsedRequestBodyKey 保存 handler 级复用的已解码请求体，避免热路径重复解析。
...
func setGatewayRequestRanges(parsed *ParsedRequest, protocol string, body []byte, jsonStr string) error {
	if parsed == nil {
		return nil
	}
	switch protocol {
	case domain.PlatformGemini:
		if sysParts := gjson.Get(jsonStr, "systemInstruction.parts"); sysParts.Exists() && sysParts.IsArray() {
			...
			if err := json.Unmarshal(sliceRawFromBody(body, sysParts), &parts); err != nil {
				return fmt.Errorf("invalid systemInstruction.parts: %w", err)
			}
			parsed.System = parts
		...
		if contents := gjson.Get(jsonStr, "contents"); contents.Exists() && contents.IsArray() {
			...
			if err := json.Unmarshal(sliceRawFromBody(body, contents), &msgs); err != nil {
				return fmt.Errorf("invalid contents field: %w", err)
			}
	...
	default:
		if sys := gjson.Get(jsonStr, "system"); sys.Exists() {
			...
			default:
				...
				if err := json.Unmarshal(sliceRawFromBody(body, sys), &system); err != nil {
					return fmt.Errorf("invalid system field: %w", err)
				}
		...
		if msgs := gjson.Get(jsonStr, "messages"); msgs.Exists() && msgs.IsArray() {
			...
			if err := json.Unmarshal(sliceRawFromBody(body, msgs), &messages); err != nil {
				return fmt.Errorf("invalid messages field: %w", err)
			}
...
// normalizeClaudeCodeLongContextModel 移除 Claude Code 泄漏到请求体末尾的长上下文选择后缀。
...
func parseGatewayRequestCurrentBody(parsed *ParsedRequest, protocol string) error {
	...
	return setGatewayRequestRanges(parsed, protocol, bodyBytes, jsonStr)
}
...
type ParsedRequest struct {
	...
	System          any             // system 字段内容（兼容旧调用方；热路径优先用 SystemRaw）
	Messages        []any           // messages 数组（兼容旧调用方；热路径优先用 MessagesRaw）
...
func (p *ParsedRequest) SystemRaw() []byte {
	if raw := p.raw(p.systemRange); len(raw) > 0 {
		return raw
	}
	if p == nil {
		return nil
	}
	if p.HasSystem && p.System == nil {
		return []byte("null")
	}
	if p.System == nil {
		return nil
	}
	data, err := json.Marshal(p.System)
	if err != nil {
		return nil
	}
	return data
...
func (p *ParsedRequest) MessagesRaw() []byte {
	if raw := p.raw(p.messagesRange); len(raw) > 0 {
		return raw
	}
	if p == nil || p.Messages == nil {
		return nil
	}
...
// sanitizeAnthropicBodyForBetaTokens 根据最终发送的 anthropic-beta header
// 删除不被上游能力开关接受的 beta 字段，避免 body 与 header 不对称。
func sanitizeAnthropicBodyForBetaTokens(body []byte, anthropicBetaHeader string) ([]byte, bool) {
	...
	if b, err := sjson.DeleteBytes(body, "context_management"); err == nil {
		...
		logger.LegacyPrintf("service.gateway",
			"[CtxMgmtSanitize] sjson.DeleteBytes failed unexpectedly: %v (body len=%d). body and final anthropic-beta header may be out of sync.",
			err, len(body))
	}
	return body, false
}
