...
var (
	...
	// fingerprintUserAgentPattern 校验可持久化的产品名与三段版本号。
...
// isAcceptableFingerprintUserAgent 判断 User-Agent 是否可作为账号级持久身份。
func isAcceptableFingerprintUserAgent(ua string) bool {
	...
	return !currentOK || major <= currentMajor+maxClaudeCLIMajorVersionSkew
}
