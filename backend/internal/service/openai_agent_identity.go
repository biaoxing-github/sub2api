...
// applyOpenAIChatGPTAccountHeaders adds account-scoped headers shared by all
// ChatGPT internal API calls. FedRAMP is only meaningful for Agent Identity
// credentials and is intentionally omitted for ordinary OAuth accounts.
func applyOpenAIChatGPTAccountHeaders(headers http.Header, account *Account) {
	...
	fedRAMP, _ := account.Credentials["chatgpt_account_is_fedramp"].(bool)
	if !fedRAMP {
		fedRAMP = strings.EqualFold(strings.TrimSpace(account.GetCredential("chatgpt_account_is_fedramp")), "true")
	}
	if account.IsOpenAIAgentIdentity() && fedRAMP {
