...
const DefaultCSPPolicy = "default-src 'self'; script-src 'self' __CSP_NONCE__ https://challenges.cloudflare.com https://static.cloudflareinsights.com https://*.stripe.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://static.airwallex.com https://checkout.airwallex.com https://static-demo.airwallex.com https://checkout-demo.airwallex.com; img-src 'self' data: https:; font-src 'self' data: https://fonts.gstatic.com; connect-src 'self' https:; frame-src 'self' https://challenges.cloudflare.com https://*.stripe.com https://checkout.airwallex.com https://checkout-demo.airwallex.com; frame-ancestors 'self'; base-uri 'self'; form-action 'self'"
...
// WebAuthnConfig 定义当前部署作为 WebAuthn relying party 的固定边界。
// RP ID 与允许的 Origin 必须来自部署配置，不能从客户端请求头推断。
...
type BillingConfig struct {
	...
	// UserPlatformQuotaCacheTTLSeconds 用户 × 平台 quota 缓存 TTL（秒）。
	UserPlatformQuotaCacheTTLSeconds int `mapstructure:"user_platform_quota_cache_ttl_seconds"`
	// UserPlatformQuotaSentinelTTLSeconds 无 limit sentinel entry 的 TTL（秒）。
...
type GatewayConfig struct {
	...
	// OpenAIFirstOutputTimeoutSeconds: 原生 HTTP Responses 首个语义输出超时（秒），0 表示关闭。
	OpenAIFirstOutputTimeoutSeconds int `mapstructure:"openai_first_output_timeout_seconds"`
	// OpenAIHighEffortFirstOutputTimeoutSeconds: high/xhigh/max 推理首输出超时（秒），0 表示沿用标准值。
...
// configureConfigSource 配置 Viper 的文件来源。CONFIG_FILE 为显式文件路径，
// 一旦提供就不再搜索 DATA_DIR 或默认目录。
...
func setDefaults() {
	...
	// WebAuthn / Passkey 需要部署方显式声明 RP 域名和可信浏览器 Origin。
	...
	viper.SetDefault("gateway.openai_request_header_timeout_seconds", 90)
	...
	viper.SetDefault("gateway.codex_wait_guard.max_header_wait_seconds", 90)
	...
	// 默认 sync：队列满时由提交方内联执行，避免计费/日志任务被静默丢弃。
...
func (c *Config) Validate() error {
	...
	if c.Gateway.OpenAIWS.SchedulerScoreWeights.Priority < 0 ||
		...
		c.Gateway.OpenAIWS.SchedulerScoreWeights.Balance < 0 {
	...
	weightSum := c.Gateway.OpenAIWS.SchedulerScoreWeights.Priority +
		...
		c.Gateway.OpenAIWS.SchedulerScoreWeights.Balance
	if weightSum <= 0 {
