...
func TestRedactCredentials_StripsSensitiveKeysAndReportsStatus(t *testing.T) {
	in := map[string]any{
		...
		"api_keys_disabled":     map[string]any{"sha256:test": map[string]any{"reason": "insufficient_balance"}},
	...
	require.True(t, status["has_api_keys"])
...
func TestRedactCredentials_AllKnownSensitiveKeys(t *testing.T) {
	keys := []string{
		...
		"api_key", "api_keys", "api_keys_disabled", "session_key", "cookie",
