...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	if !gjson.ValidBytes(body) {
		reqLog.Warn("openai_alpha_search.invalid_json")
	...
	for {
		...
		accountSlot := h.acquireResponsesAccountSlot(c, apiKey.GroupID, sessionHash, selection, false, &streamStarted, reqLog)
		...
		accountRelease := accountSlot.ReleaseFunc
