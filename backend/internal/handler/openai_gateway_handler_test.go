...
func TestOpenAIHandleStreamingAwareErrorWithCode_EmitsStableClassification(t *testing.T) {
	...
	jsonBody := body[strings.Index(body, "{"):]
	require.Equal(t, "upstream_error", gjson.Get(jsonBody, "error.type").String())
	require.Equal(t, service.OpenAIUpstreamHTTP2StreamErrorCode, gjson.Get(jsonBody, "error.code").String())
...
func TestResolveOpenAIMessagesMetadataSession_ClaudeCodeHeaderOverridesContentFallback(t *testing.T) {
	...
	contentHash1 := (&service.OpenAIGatewayService{}).GenerateSessionHashWithFallback(c, body1, string(body1))
	contentHash2 := (&service.OpenAIGatewayService{}).GenerateSessionHashWithFallback(c, body2, string(body2))
