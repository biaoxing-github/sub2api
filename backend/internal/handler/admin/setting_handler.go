...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.CodexStabilityMode != nil {
		...
		switch normalized {
		case config.GatewayCodexStabilityModeOff, config.GatewayCodexStabilityModeCodex, config.GatewayCodexStabilityModeAllOpenAIResponses:
		default:
			response.Error(c, http.StatusBadRequest, "codex_stability_mode must be off, codex, or all_openai_responses")
			return
		}
	}
	...
	settings := &service.SystemSettings{
		...
		CodexStabilityMode: func() string {
			...
			return previousSettings.CodexStabilityMode
		}(),
		CodexStabilityDynamicHeaderTimeoutEnabled:  boolValueOrDefault(req.CodexStabilityDynamicHeaderTimeoutEnabled, previousSettings.CodexStabilityDynamicHeaderTimeoutEnabled),
...
func diffSettings(before *service.SystemSettings, after *service.SystemSettings, beforeAuthSourceDefaults *service.AuthSourceDefaultSettings, afterAuthSourceDefaults *service.AuthSourceDefaultSettings, req UpdateSettingsRequest) []string {
	...
	if before.CodexStabilityDynamicHeaderTimeoutEnabled != after.CodexStabilityDynamicHeaderTimeoutEnabled {
		changed = append(changed, "codex_stability_dynamic_header_timeout_enabled")
	}
	if before.CodexStabilityRequestPhaseFailoverEnabled != after.CodexStabilityRequestPhaseFailoverEnabled {
		changed = append(changed, "codex_stability_request_phase_failover_enabled")
	}
	if before.CodexStabilitySuppressClientTimeoutHeaders != after.CodexStabilitySuppressClientTimeoutHeaders {
		changed = append(changed, "codex_stability_suppress_client_timeout_headers")
	}
...
// resolveSMTPUseTLS 区分未提交 TLS 配置与显式关闭，未提交时使用已保存值。
