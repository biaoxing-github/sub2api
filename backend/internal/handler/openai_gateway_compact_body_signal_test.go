...
func TestNormalizeOpenAIResponsesCompactRequest_RejectsInvalidStreamType(t *testing.T) {
	h := &OpenAIGatewayHandler{}
	for name, body := range map[string][]byte{
		"string": []byte(`{"model":"gpt-5.5","stream":"true","input":[{"type":"compaction_trigger"}]}`),
		"number": []byte(`{"model":"gpt-5.5","stream":1,"input":[{"type":"compaction_trigger"}]}`),
	} {
