...
func TestGatewayModels_AnthropicCustomModelsListIncludesOAuthClaudeAndMappedModel(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{
			...
			ModelsListConfig: service.GroupModelsListConfig{
				Enabled: true,
				Models:  []string{"claude-fable-5", "claude-opus-4-6-thinking", "deepseek-v4-pro"},
			},
		},
	})
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"claude-fable-5", "claude-opus-4-6-thinking", "deepseek-v4-pro"}, modelIDsForTest(got.Data))
}
