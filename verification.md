...
## 2026-07-15 - v0.1.156 第二阶段融入完成

- 执行者：Devil。
- PASS：取消后的请求不再继续切换账号或误报 502；未提交响应按 499 归类。
- PASS：additional-tools、Codex 图片 function tool、Grok 图片端点和 Antigravity refresh token 四项兼容修复已融入。
- PASS：后端聚焦测试、service 全量测试、全包编译、前端 Vitest 和类型检查通过。
...
- 执行者：Devil。
- PASS：普通页面读取优先使用 PostgreSQL 缓存；缓存缺失账号首次实时拉取并写入，未引用且已有缓存的账号不再访问上游。
- PASS：数据库已引用账号在普通读取时实时刷新；手动“同步 Key / 分组”通过独立 POST 接口全量刷新全部启用账号。
- PASS：追加、替换完整 Key 前实时刷新并回写缓存；修改分组成功后重新拉取账号最新 Key 与分组并回写缓存。
...
- 执行者：Devil。
- PASS：平台目录新增“余额 / 已用”列，按站点与用户 ID 精确关联数据库缓存记录。
- PASS：有缓存时显示当前余额和累计已用，悬停可查看缓存更新时间；无缓存显示“暂无缓存”。
