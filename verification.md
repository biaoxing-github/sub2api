...
- 执行者：Devil。
- PASS：调度池可直接编辑账号优先级，确认按钮仅提交 `priority` 并刷新池快照。
- PASS：调度顺序为最低优先级硬分层；同优先级内先比较剩余并发槽，再综合负载率、排队、健康度、延迟与 LRU。
- PASS：Go 聚焦测试、调度池 Vitest 8/8、`vue-tsc`、Vite 生产构建和 `git diff --check` 通过。
