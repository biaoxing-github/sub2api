...
- 每次发版的 `docs/releases/` 版本日志必须写“详细更新日志”：至少按条目列出相较上个版本新增/修改了什么、涉及的文件或模块、对用户可见行为的影响、验证命令和结果、回滚目标和遗留风险；只写一句摘要不算完成。`docs/feature_list.jsonl`、`docs/process_list.jsonl` 仍只作为过程流水，不能替代版本日志；切流完成后必须回填最终 active 颜色、线上镜像和验证结论。
...
- `VersionBadge` 的主版本必须直接使用 `/admin/system/version.current_version`，并统一规范为恰好一个 `v` 前缀；禁止在组件中维护独立版本常量。每次发布必须验证侧边栏按钮、版本下拉主版本和 `image_version`，不得出现旧版本或 `vv0.x.y`。
